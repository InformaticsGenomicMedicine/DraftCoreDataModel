...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Validate HGVS expressions from an Excel or CSV file."
    )
    parser.add_argument("input_file", help="Path to the input file")
    parser.add_argument("output_file", help="Path to the output CSV file")
    parser.add_argument(
        "example_column", help="Name of the column containing HGVS examples"
    )
    parser.add_argument(
        "human_curated", help="Name of the column containing human curated results"
    )

    args = parser.parse_args()

    hgvs_validator = HGVSValidator(
        args.input_file, args.output_file, args.example_column, args.human_curated
    )
